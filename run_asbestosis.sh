...
HEAD_DROPOUT="${ASBESTOSIS_HEAD_DROPOUT:-0.4}"
WEIGHT_DECAY="${ASBESTOSIS_WEIGHT_DECAY:-3e-3}"
FREEZE_BACKBONE_EPOCHS="${ASBESTOSIS_FREEZE_BACKBONE_EPOCHS:-10}"
EARLY_STOP_PATIENCE="${ASBESTOSIS_EARLY_STOP_PATIENCE:-5}"
...
if [[ "${REGENERATE_SPLITS}" == "1" ]]; then
  ARGS+=("--regenerate-splits")
fi
